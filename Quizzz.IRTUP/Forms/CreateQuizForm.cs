...
namespace Quizzz.IRTUP.Forms 
{
    public partial class CreateQuizForm : Form
    {
        ...
        private void quizScreen()
        {
            ...
            if (questionTypeComboBox.SelectedItem == "True or False")
            {
                TrueFalseControl true_Or_False = new TrueFalseControl();
                selectedQuizPanel.Controls.Clear();
                selectedQuizPanel.Controls.Add(true_Or_False);
            }
        }
        ...
        private void saveQuizBtn_Click(object sender, EventArgs e)
        {
            ...
            foreach (var entry in slides)
            {
                ...
                if (entry.Value.Control is TrueFalseControl tfControl)
                {
                    QuestionData question = new QuestionData
                    {
                        QuizID = quizID,
                        QuestionType = "TrueFalse",
                        QuestionText = tfControl.QuestionText,
                        CorrectAnswer = tfControl.SelectedAnswer,
                        QuestionNo = tfControl.QuestionNo
                    };

                    db.SaveTrueFalseQuestion(question, quizID);
                }
            }
